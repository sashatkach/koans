...
class Proxy
  ...
  def method_missing(method_name, *args, &block)
    begin
      if @messages.transpose[0].include? method_name
        @messages.each do |item|
          if(item.at(0) == method_name.to_sym)
            item[1] += 1
          end
        end
      else
      ...
      @messages.push([method_name.to_sym, 1])
    end
    if(args.length > 0)
  ...
  def messages
    @messages.transpose[0]
  end
  def called?(symbol)
    @messages.transpose[0].include?(symbol)
  end
  def number_of_times_called(symbol)
    idx = @messages.transpose[0].index(symbol)
    idx ? @messages.dig(idx, 1) : 0
  end
  # WRITE CODE HERE
end
